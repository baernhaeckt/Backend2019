...
data["Month"] = date
data["Points"] = numbers
data.to_csv("Backend.Statistic\\time_series.csv", header=True, index=False)

#%% Plot Time Series
dataset = pd.read_csv("Backend.Statistic\\data.csv", usecols=[1], header=0)
plt.plot(dataset)
plt.show()

#%% Load Data
dataframe = pd.read_csv("Backend.Statistic\\data.csv", usecols=[1], header=0)
